...
envia_a_tots_excepte(display,M,N) -> bppool ! {sent_except,display,M,N}.
...
procPisos(Pisos) -> receive
	...
	{display,N,M} -> lists:nth(N,Pisos) ! {display,M},procPisos(Pisos);
	{sent_except,display,Message,Except} -> envia_a_tots_excepte(Pisos,display,{display,Message},Except,1),procPisos(Pisos);
	{set_light,Pis,all,on} -> lists:nth(Pis,Pisos) ! {light_on,up}, lists:nth(Pis,Pisos) ! {light_on,down}, procPisos(Pisos);
	{set_light,Pis,all,off} -> lists:nth(Pis,Pisos) ! {light_off,up}, lists:nth(Pis,Pisos) ! {light_off,down},procPisos(Pisos);
	{set_light,Pis,N,on} -> lists:nth(Pis,Pisos) ! {light_on,N},procPisos(Pisos);
	{set_light,Pis,N,off} -> lists:nth(Pis,Pisos) ! {light_off,N},procPisos(Pisos);
	kill -> envia_a_tots(Pisos,kill), killed

end.

create(1) -> [bpis1:new(1)];
create(N) -> create(N-1) ++ [bpis1:new(N)]. 

start(N) -> LlistaPisos=create(N),register(bppool,spawn(?MODULE,procPisos,[LlistaPisos])).
