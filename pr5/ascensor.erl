...
ascensorProc(e1,Button,[]) -> receive
	{sens_pl, Button} -> stop(),io:format("No hi ha pisos pendents, acabem amb ~p~n",[Button]),display(Button),light_off(Button), ascensorProc(Button);
...
ascensorProc(BotoAct) -> receive
	...
	{clicked,K} when K > BotoAct -> io:format("Comencem amb el PIS ~p~n",[K]),ascensorProc(e0,up,K);
